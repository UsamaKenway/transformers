...
def load_gguf_checkpoint(gguf_checkpoint_path, return_tensors=False, model_to_load=None, torch_dtype=None):
    ...
    # Gemma4 GGUF checkpoints only contain the text backbone.
    # Several config values are stored as per-layer arrays in GGUF but the HF
    # config expects scalars plus boolean flags; convert them here.
